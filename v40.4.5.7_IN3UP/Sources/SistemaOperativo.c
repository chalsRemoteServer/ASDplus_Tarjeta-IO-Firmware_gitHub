		...
		switch(estado3){//Maquina de Estados
			...
			case 7:if(vfd.bits.b.TxBuffOFF)//buffer de Tx vacio?
					 if(!menu.b.b.isBusy)//No se esta graficando  	  
					       estado3++;
				   break;
			case 8://if(delay_us_v3(100,&waitsDDS))
				   estado3++;break;
...
void vTask3_ImAlive_Transm_al_CPU_TFT(void){
...
l73:    switch(estado72){
	    	case 1:if(++count>80000)estado72++;break;
	    	...
	    	case 5:if(vfd.TxDisp.appendByte(datos[i],&vfd.TxDisp))estado72++;break;
	    	case 6:if(++i<SIZE_TRAMA)estado72--;
	    	       else{estado72++;}break;
			case 7:Monitor_System_status_LEDs(CMD_IM_ALIVE);estado72++;break;
...
unsigned char *x;
    ...
    switch(estado){
    	...
    	case 2:if(vfd.TxDisp.popf(x,&vfd.TxDisp)){estado++;}break;
