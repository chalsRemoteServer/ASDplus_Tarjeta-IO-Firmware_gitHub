...
#define ERROR_RELOJ_ANALOGO                  0x41

/* fin de definiciones comunnes de IO  y UP  */
// Definiciones de tiempos para LED System (en milisegundos)
#define LED_SYSTEM_NORMAL_ON_TIME        280      // ms ON
#define LED_SYSTEM_NORMAL_OFF_TIME       9     // ms OFF
...
#define LED_SYSTEM_ERROR_ON_TIME         300  // ms ON (aproximado)
#define LED_SYSTEM_ERROR_OFF_TIME        300  // ms OFF (aproximado)

// Definiciones de tiempos para LED Warning
#define LED_WARNING_NORMAL_ON_TIME       300      // 50ms ON
#define LED_WARNING_NORMAL_OFF_TIME      8     // 700ms OFF

#define LED_WARNING_SYSMON_ON_TIME       500     // 500ms ON
#define LED_WARNING_SYSMON_OFF_TIME      500     // 500ms OFF

#define LED_WARNING_COMM_ON_TIME       50     // 500ms ON estan al revez el on off en warnning
#define LED_WARNING_COMM_OFF_TIME      800     // 500ms OFF
...
//	unsigned char position[100];
	union _LEDS{
     ...
     union _ERROR{
     	    unsigned char LEDS;
     		struct {
     	    unsigned char Display:1;//no detectamos display 
     		unsigned char Teclado:1;//No detectamos Teclado
     		unsigned char IO:1;   //No detectamos Tarjeta IO
     		unsigned char PC:1;   //No hay comunicacion con PC
     		unsigned char DDS_:1;  //No detectamos Frecuencias DDS
     		unsigned char ADC_:1;  //No detectamos ADC trabajando
     		unsigned char EEPROM_:1;//No detectamos EEPROM
     		unsigned char NVRAM:1; //No detectamos NVRAM 
     	   }bits;
       }Error13;
  struct _Leds{
	unsigned short int ErrorCount;//no asignado 
	unsigned char  estado;
	}leds;	
  struct _PRIORIDAD{
		unsigned char estado;//estado del metodo error prioridad
		unsigned short timer;//timer de delays de error de prioridad
	}priority;	
	
  struct __Status{
	 unsigned char estado; //maquina de estados del controlador
	 unsigned short int delay;//delay de la maquina de estados
	 unsigned short int timer;//se incrementa en T1
  }status;
...
void Monitor_System_status_LEDs(unsigned char status);
