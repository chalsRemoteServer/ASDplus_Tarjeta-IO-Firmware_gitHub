...
/* MUY IMPORTANTE CONFIGURACION  configuracion del AD9833
 * input buffer 0
 * output bufer 22
 * Wdth=8bits
 * Clock Edge=falling edge
 ...
 * 
 * */
unsigned char init_Control_deFrecuencias(unsigned char *memo){	
...
//const unsigned short int waveform=0x2000; //forma de onda sinusoidal PIN16=disable
const unsigned short int waveform=0x2020;//forma sinusoidal y signout=frec div/2
unsigned char i;
...
unsigned char *estado123,ret=FALSE;
unsigned short int *contador;
const unsigned short int DELAY_CONFIG=12000;//Aprox=1000milisec
//0x00–0x0F Transmit RAM
//0x10–0x1F Receive RAM
//0x20–0x2F Command RAM
//unsigned long int delay=0;
  estado123=memo;//memoria del sub proceso
  contador=(unsigned short int*)&memo[1];//apunta a bytes 1-2
  switch(*estado123){
  case 10:
     //TI1_Disable(); Timer 1 sigue funcionando en esta version
     Select_SPI_device(DDS);//seleccionamos el DDS para la comunicacion SPI
     setReg16(PQSPAR,0x1515);//0001 0101 0001 0101=1515H
     setReg16(QMR, 0x0000);//se resetea el QSPI 0x0000 Resetea el módulo QSPI a su estado por defecto
     setReg16(QMR,_MSTR|_BAUD | _16BITS);//config QSPI, CPOL=0,CPHA=1
     setReg16(QWR,0x0000); //reset QWR  QSPI Wrap Register
     setReg16(QWR,0x0500);//setup queue begin at 0 end to 6,Configura la cola QSPI de 0 a 5  
    ...
    setReg16(QDR,0x7E00);//0111 palabra de reset, primer comando
     for(i=1;i<6;i++)
        setReg16(QDR,0x6000);//1000 0000 0000 0000, SPIF, carga  comandos nop
     setReg16(QAR,0x0000);//direccion zero de Transmission
     for(i=0;i<6;i++)
    	 setReg16(QDR,w[i]);//datos a transmitir,calculada en get_the_words_
     setReg16Bits(QIR,0x0010);//_SPIFE);QSPI finished (SPIF) interrupt enable.
     FSYNC_PutVal(FALSE);//baja=empieza transferencia SPI
     setReg16Bit(QDLYR,SPE);//start trasmit to DDS
     setReg16(QIR,_SPIF);//clear the int Flag
     //for(delay=0;delay<210;delay++)__asm(nop);//Fsys=49.76Mhz 
     (*estado123)++;*contador=0;break;
  case 11:   
     if(getReg16(QIR)==_SPIF){(*estado123)++;// cuando termine todo el wrap
         Monitor_System_status_LEDs(ERR_SEVERO_AD9833_CLEAR);}
     else{if((*contador)++<DELAY_CONFIG){*contador=0;
    	 	 Monitor_System_status_LEDs(ERR_SEVERO_AD9833);
    	 	FSYNC_PutVal(TRUE);//sube= no hay transferencia SPI
             *estado123=10;}}
     break;
  case 12:
     FSYNC_PutVal(TRUE);//sube= no hay transferencia SPI
     ret=TRUE;break;  	  
  default:*estado123=10;break;}
return ret;     
...
//unsigned int i;
 ...
 /* TArea qeue genera la señal DDS y monitorea que funcione*/
void vTask6_Direct_Digital_Synthetizador_Y_Monitor(void){
static unsigned char estado6;	 
static unsigned char mem[3];//3 bytes de memoria hasta ahora
    switch(estado6){
    	case 10:ddsgen.Status=0;estado6++;break;//no init no frec gen
    	case 11:if(init_Control_deFrecuencias(&mem[0]))estado6++;break;
    	case 12:Enable_ADCs();estado6++;break;
    	case 13:if(monitor_Reloj_Analogo(&ddsgen,&mem[0]))estado6++;break;
    	case 14:if(monitor_Driver_Signal(&ddsgen,&mem[0]))estado6++;break;
    	case 15:if(ddsgen.status)
    		   switch(ddsgen.Status){
    		     case CAMBIO_FRECQ:disable_ADC();estado6=10;break;
    		     case ERROR_CLK_ANALOGO:disable_ADC();estado6=10;break;
    		     case ERROR_
    	}
    	default:}

init_Control_deFrecuencias
}//fin del control y monitor del DDS-------------------------------
