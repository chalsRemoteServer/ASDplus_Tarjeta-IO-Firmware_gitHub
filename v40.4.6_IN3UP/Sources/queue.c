...
struct DDS_GEN ddsgen;
struct _SysMon sysmon,sysmon2;
struct _Recursos recurso;
struct COMUNICACIONES_ com;
fifo_manager_t fifo;// Definición de la variable global de FIFOs

// Buffers estáticos para cada FIFO
// Ajusta los tamaños según necesites
#define FIFO_VFDSERIAL_SIZE 256
static uint8_t vfdserial_buffer[FIFO_VFDSERIAL_SIZE];
static unsigned char recurso_VFD_Tx;//recurso cola para transmision serial a tarjeta display
...
unsigned char  buffer13[SIZE_BUFFER13];//BUffer13 fifo de funciones que espera recurso VFD
unsigned char  buffer14[SIZE_BUFFER14];//buffer de la fifo de transmision de tx de serial a IOUP 
unsigned char  buffer15[SIZE_BUFFER15];//buffer de xL, xlower byte, deteccion a IOUP
unsigned char  buffer16[SIZE_BUFFER15];//buffer de xH, xHi byte, deteccion a IOUP
unsigned char  buffer17[SIZE_BUFFER15];//buffer de yL, ylower byte, deteccion a IOUP
...
unsigned char  buffer22[SIZE_BUFFER22];//fifo de recursos de tx serial
...
void init_queues(void){
	//unsigned char c;
    	  ...
    	  init_FIFO_General_1byte(&serial.rx,&buffer2[0],&buffer2[SIZE_BUFFER1-1],SIZE_BUFFER1);
     	  ...
     	  init_FIFO_General_1byte(&recurso.VFDprocesos,&buffer13[0],&buffer13[SIZE_BUFFER13-1],SIZE_BUFFER13);
     	  init_FIFO_General_1byte(&com.IOUP.tx,&buffer14[0],&buffer14[SIZE_BUFFER14-1],SIZE_BUFFER14);
     	  init_FIFO_General_1byte(&com.gotis.dds.fifos.xl,&buffer15[0],&buffer15[SIZE_BUFFER15-1],SIZE_BUFFER15);
     	  init_FIFO_General_1byte(&com.gotis.dds.fifos.xh,&buffer16[0],&buffer16[SIZE_BUFFER15-1],SIZE_BUFFER15);
     	  init_FIFO_General_1byte(&com.gotis.dds.fifos.yl,&buffer17[0],&buffer17[SIZE_BUFFER15-1],SIZE_BUFFER15);
     	  ...
     	  init_FIFO_General_1byte_v2(&recurso.COMM_IO,&buffer22[0],SIZE_BUFFER22);     	  
     	      	  
     	  recurso.COMM_IO.mode=1;//activado modo debug
     	  fifo_system_init();    // Reservar memoria para las estructuras FIFO
          ...
          vfd.bits.b.finit_VFD=FALSE;//no esta inizilizado el VFD, se inizializa en el driver de video
     	  vfd.bits.b.finit_Menu=FALSE;//no esta inizializado el menu
//          vfd.v.estado=0;//init_vfd
          sysmon.reset=reset_sys_mon;
          sysmon2.reset=reset_sys_mon2;
//          recurso.solicitar=Recursos_Solicitud;
//          recurso.devolver =Recursos_Devolver;
          recurso.v.VFD_pid=0;
//          recurso.get_inst=get_instancia;
          recurso.dev_inst=return_instancia;
          recurso.insertVFDProcess=insertVFDProcess_;
          recurso.solicitar=Solicitar_Recurso;
          recurso.devolver=Devolver_Recurso_VFD;
          com.gotis.dds.f.fpop=xpop_FIFO_CMD_DDS;
          com.gotis.dds.f.fpush=push_FIFO_CMD_DDS;
//          TBC.ret.RetByte=0;//return control todos en cero
          menu.ret.u.RetByte=0;//todos los return a zero.
...
// Inicializar todas las FIFOs del sistema
void fifo_system_init(void) {    // Reservar memoria para las estructuras FIFO
    static fifo_t vfdserial_fifo;
    fifo.VFDserial = &vfdserial_fifo;    // Asignar punteros
    // Inicializar cada FIFO con su buffer
    fifo_init(fifo.VFDserial, vfdserial_buffer, FIFO_VFDSERIAL_SIZE);
}//-----------------------------------------------------------------------


// Inicializar una FIFO
void fifo_init(fifo_t *f, uint8_t *buffer, uint16_t size) {
    ...
    f->count = 0;
    f->overflow = false;
    memset(buffer, 0, size);    // Limpiar buffer (opcional)
}//-----------------------------------------------------



// Agregar un dato a la FIFO
...
// Limpiar bandera de desbordamiento
  ...
  if(s->ncount < s->size){ 
    if(s->mode==0){//normal
    	...
    	*(s->push) = dato;
    	s->push++;
    	s->ncount++;}}
  if(s->push>s->tail){__asm(nop);//error de software o pila
  	  	  	  	  	  __asm(Halt);}
return ret;
}//fin de push en fifo------------------------------------------------
...
unsigned char i;   
    sysmon.size=SIZE_SYS_MONITOR;
	for(i=0;i<sysmon.size;i++)
		sysmon.t[i]=0;
}//---------------------------------------

void reset_sys_mon2(void){
unsigned char i;   
    sysmon2.size=SIZE_SYS_MONITOR;
	for(i=0;i<sysmon2.size;i++)
		sysmon2.t[i]=0;
}//---------------------------------------
...
/*  inisializacion del monitor del systema */
...
//uword3 v;	
		...
		VFDserial_SendChar1(*(p+0)+0x30); //decmil+0x30);
		VFDserial_SendChar1(*(p+1)+0x30);//mil+0x30);
		VFDserial_SendChar1(*(p+2)+0x30);//cent+0x30);
		VFDserial_SendChar1(*(p+3)+0x30);//dec+0x30);
		VFDserial_SendChar1(*(p+4)+0x30);//unidad+0x30);	  
	   // if(n>0)
...
/* funcion para meter en la FIFO de comMando de DDS para 
 * transmitir el DDS al touchscreen  
 * Se meten los datos a una FIFO provenientes de la deteccion }
 * para ser desplegados en el Gotis 
 *  ************************************************************ */
...
union{
 ...
 struct _X1_{
    ...
    if(search_NO_Repetido(x,y)){
		u.ssint=x;
		ret+=com.gotis.dds.fifos.xl.appendByte(u.b.L,&com.gotis.dds.fifos.xl);
		ret+=com.gotis.dds.fifos.xh.appendByte(u.b.H,&com.gotis.dds.fifos.xh);
		u.ssint=y;
		ret+=com.gotis.dds.fifos.yl.appendByte(u.b.L,&com.gotis.dds.fifos.yl);
		ret+=com.gotis.dds.fifos.yh.appendByte(u.b.H,&com.gotis.dds.fifos.yh);}
if(ret==4)
	return TRUE;
else return FALSE;
...
unsigned char xpop_FIFO_CMD_DDS(signed short int *x,signed short int *y){
	...
	union{
	 ...
	 struct _X1_{
			  unsigned char H;
			  unsigned char L;}b;}u;	
	    ret+=com.gotis.dds.fifos.xl.popf(&u.b.L,&com.gotis.dds.fifos.xl);
	    ret+=com.gotis.dds.fifos.xh.popf(&u.b.H,&com.gotis.dds.fifos.xh);
	    *x=u.usint;
	    ret+=com.gotis.dds.fifos.yl.popf(&u.b.L,&com.gotis.dds.fifos.yl);
	    ret+=com.gotis.dds.fifos.yh.popf(&u.b.H,&com.gotis.dds.fifos.yh);
	    *y=u.usint;
if(ret==4)
	return TRUE;
else return FALSE;
...
//					pushFIFOcOP_inicioCMD();
  ...
  //angulo=22;sens=25;        Angulo,sens
//  display_Sensibilidad_Limites(22,25);
...
unsigned char ii,jj,kk;
   ...
   for(m=0;m<2;m++){
	vfd.f1.resetFIFOS();
	for(i=0,j=1,k=2;i<SIZE_BUFFER6-2;i++,j++,k++){
	       if(vfd.x.ncount!=i){//vfd.x.ncount
	    	     __asm(nop);
	    	     __asm(Halt);
	             __asm(nop);}
	       if(!vfd.f1.append(i,j,k))
	    	      break;  
	        }
	for(i=0,j=1,k=2;i<SIZE_BUFFER6-3;i++,j++,k++){
	       vfd.f1.pop(&ii,&jj,&kk);
	       if(!((i==ii)&&(j==jj)&&(k==kk))){
	    	       __asm(nop);
	    	       __asm(Halt);}}
    
    for(i=0;i<SIZE_BUFFER16;i++){
	    if(!recurso.VFD.appendByte(i+1,&recurso.VFD)){
	    	   __asm(nop);__asm(Halt);__asm(nop);}   
	for(i=0;i<SIZE_BUFFER16;i++){
		 if(recurso.VFD.popf(&j,&recurso.VFD)){
			if(j!=i+1){ 
		    	  __asm(nop);__asm(Halt);__asm(nop);}}}}}
...
//en el FIFO de los procesos de VFD se inseta el proceso
/* se pregunta su ya esta en la fifo si no esta se inserta
 * Antes de insertar verifica si ya esta insertado sino lo inserta*/
unsigned char insertVFDProcess_(unsigned char process){
unsigned char ret=0,i;	
    for(i=0;i<SIZE_BUFFER13;i++){
    	  if(buffer13[i]==process){
    		    ret=TRUE;break;}}//si esta el proceso en fila
    if(!ret){recurso.VFDprocesos.appendByte(process,&recurso.VFDprocesos);}
return ret;    
}//fin insert VFD process--------------------------------------


/* return true if was a resource asigned
 * false if there is no source asigned or if it is in waiting fifo
 * 
 * PID4 tiene prioridad sobre otros cuando haya un menu pendiente */
unsigned char Solicitar_Recurso(unsigned char Recurso,const unsigned char pid){
auto unsigned char ret=0;	
auto unsigned char recurso1;
  


switch(Recurso){
case _VFD_:if(recurso.v.asignado==pid){ 
	         if(menu.b.b.MenuPendiente){ 
	        	 if(recurso.v.asignado==PID4) ret=TRUE;
	        	 else{recurso.v.asignado=0;ret=0;}}
	         else{ret=TRUE;}}
           else{switch(recurso.v.asignado){
				  case 0://esta libre el recurso?
					       if(menu.b.b.MenuPendiente){
					    	    if(pid==PID4){recurso.v.asignado=PID4;ret=TRUE;break;}
					    	    else{ret=FALSE;break;}}
						   if(recurso.VFD.popf(&recurso1,&recurso.VFD)){//sacamos el pid que sigue
							   recurso.v.asignado=recurso1;//lo asignamos al que seguia
							   while(!sacar_de_la_FIFO_vfd(recurso1)) __asm(nop);}//sacar de la lista 
						   else{recurso.v.asignado=pid;  //no hay pid que solicite recursos 
								ret=TRUE;}
						   break;
				  default:if(pid==PID4){ret=FALSE;break;}//no se mete a la fifo porque se duplicara
					      if(!ya_esta_en_la_FIFO_VFD(pid)){//el recurso ya esta asignado Y esta en lla fifo?  
							  if(recurso.VFD.appendByte(pid,&recurso.VFD)){
								  while(!insertarlo_en_Fifo_VFD(pid))__asm(nop);}}
						  break;}}
           break;//fin de VFD case ---------------------------------------
case _IO_:if(recurso.v.asignadoIO==pid){ 
             if(menu.b.b.MenuPendiente){ 
   	           if(recurso.v.asignadoIO==PID4) ret=TRUE;
   	           else{recurso.v.asignadoIO=0;ret=0;}}
             else{ret=TRUE;}}
          else{switch(recurso.v.asignadoIO){
		        case 0://esta libre el recurso?
			          if(menu.b.b.MenuPendiente){
			    	     if(pid==PID4){recurso.v.asignadoIO=PID4;ret=TRUE;break;}
			    	     else{ret=FALSE;break;}}
				      if(recurso.IO.popf(&recurso1,&recurso.IO)){//sacamos el pid que sigue
					     recurso.v.asignadoIO=recurso1;//lo asignamos al que seguia
					     while(!sacar_de_la_FIFO_IO(recurso1)) __asm(nop);}//sacar de la lista 
				      else{recurso.v.asignadoIO=pid;ret=TRUE;}  //no hay pid que solicite recursos 
   				     break;
		        default:if(pid==PID4){ret=FALSE;break;}//no se mete a la fifo porque se duplicara
			            if(!ya_esta_en_la_FIFO_IO(pid)){//el recurso ya esta asignado Y esta en lla fifo?  
					        if(recurso.IO.appendByte(pid,&recurso.IO)){
						         while(!insertarlo_en_Fifo_IO(pid))__asm(nop);}}
				        break;}}
          break;//fin de VFD case ---------------------------------------           
...
/* regresa TRUE si ya esta el pid insertado en la fifo
 * FALSE: si el PID  no este en la FIFO */
unsigned char ya_esta_en_la_FIFO_VFD(unsigned char pid){
unsigned char ret=0;
const unsigned short int byt[]={0x0001,0x0002,0x0004,0x0008,//1,2,3,4
								0x0010,0x0020,0x0040,0x0080,//5,6,7,8,
								0x0100,0x0200,0x0400,0x0800,//9,10,11,12
								0x1000,0x2000,0x4000,0x8000};//13,14,15,16
  if((recurso.v.control&byt[pid])==byt[pid]){ret=TRUE;}
return ret;
}//fin de preguntar si ya esta solicitado el recurso
...
unsigned char ya_esta_en_la_FIFO_IO(unsigned char pid){
	...
	const unsigned short int byt[]={0x0001,0x0002,0x0004,0x0008,//1,2,3,4
									...
									0x0100,0x0200,0x0400,0x0800,//9,10,11,12
									0x1000,0x2000,0x4000,0x8000};//13,14,15,16
	  if((recurso.v.control_IO&byt[pid])==byt[pid]){ret=TRUE;}
return ret;
}//fin de preguntar si ya esta solicitado el recurso


/* obtiene los parametros que dice que registro
 * pertences el pid donde se guarda el status si esta o no 
 en al fifo de vfd*/
void get_parameters_to_process_PID(unsigned char *pid1,unsigned char *index,unsigned char pid){
const unsigned char byt[]={1,2,4,8,16,32,64,128};
  
	if(pid==0){
		    __asm(nop);__asm(Halt);}
	   if(pid<9){*pid1=byt[pid];
		    *index=0;}
	   else{if(pid<17){*pid1=byt[pid-9];
	            *index=1;}
	   	   	else{if(pid<25){*pid1=byt[pid-17];
	   	   	         *index=2;}
	   	   	     else{if(pid<33){*pid1=byt[pid-25];
	   	   	               *index=3;}
	   	   	          else{__asm(nop);//ya no hay mas pid, error de doftware
	   	   	                __asm(Halt);
	   	   	           	    __asm(nop);}}}}
}//fin de obtener los parametros para procesar el PID que indica si se trabajo en la fifo con el


unsigned char sacar_de_la_FIFO_vfd(unsigned char pid){
auto unsigned char ret=0;
  unsigned short int byt[]={0x0001,0x0002,0x0004,0x0008,
							0x0010,0x0020,0x0040,0x0080,
							0x0100,0x0200,0x0400,0x0800,
							0x1000,0x2000,0x4000,0x8000};
			
	  recurso.v.control&=~byt[pid];
	  if((recurso.v.control&byt[pid])==0)
		  ret=TRUE;
return ret;
}//sacar de la lista de las fifos el pid que se soliciata------------
...
auto unsigned char ret=0;
	  unsigned short int byt[]={0x0001,0x0002,0x0004,0x0008,
		  ...
		  recurso.v.control_IO&=~byt[pid];
		  if((recurso.v.control_IO&byt[pid])==0)
			  ret=TRUE;
return ret;
}//sacar de la lista de las fifos el pid que se soliciata------------
...
unsigned char insertarlo_en_Fifo_IO(unsigned char pid){
	...
	unsigned short int byt[]={0x0001,0x0002,0x0004,0x0008,
		  ...
		  recurso.v.control_IO|=byt[pid];  //14 tics *17=238nseg
		  if((recurso.v.control_IO&byt[pid])==byt[pid])
			                  ret=TRUE;
return ret;
}//sacar de la lista de las fifos el pid que se soliciata------------
...
unsigned short int byt[]={0x0001,0x0002,0x0004,0x0008,
	  ...
	  recurso.v.control|=byt[pid];  //14 tics *17=238nseg
	  if((recurso.v.control&byt[pid])==byt[pid])
		                  ret=TRUE;
return ret;
}//sacar de la lista de las fifos el pid que se soliciata------------

void Devolver_Recurso_VFD(unsigned char recurso1,unsigned char pid){
	
 switch(recurso1){
	 case _VFD_:if(pid==recurso.v.asignado) 
		                recurso.v.asignado=0;
	            else{__asm(nop);__asm(Halt);}
	            break;
	 case _IO_:if(pid==recurso.v.asignadoIO) recurso.v.asignadoIO=0;
			   else{__asm(nop);__asm(Halt);}
	 	 	   break;
...
static unsigned char colaLlena(struct _FIFO_1byte_ *f) {
   return(f->ncount>=f->size) ? TRUE:FALSE;
}//--------------------------------------------------------------

// Función auxiliar para verificar si la cola está vacía
static unsigned char colaVacia(struct _FIFO_1byte_ *f) {
    return (f->ncount == 0) ? TRUE : FALSE;
}//-------------------------------------------------- ------------

// Función para verificar si un ID ya está en la cola
//return true:si se encuentra en la cola, False sino esta en lacola
static unsigned char idEnCola(struct _FIFO_1byte_ *f,unsigned char id){
unsigned char *i;
unsigned char cont = 0;    
    if(colaVacia(f)){return FALSE;}
    for(i=f->head;i<=f->tail;i++){
    	if(*i==id){return TRUE;}}
return false; 
}//-----------------------------------------------------------

// Función para encolar un ID (solo si no está ya en la cola)fifo_system_init
//RETURN true: se encolo bien sin problema
static unsigned char encolar(struct _FIFO_1byte_ *f,unsigned char id) {
    if (idEnCola(f,id)) {    // Verificar si el ID ya está en la cola
        return TRUE;}  // ID ya está en espera, no se encola de nuevo
    if (colaLlena(f)) {
        return FALSE;}  // Cola llena, no se puede encolar
    if(f->appendByte(id,f))
              return TRUE;
return FALSE;    
}//--------------------------------------------------------------

// Función para desencolar un ID, saca el id que esta en la fifo en cualquier parte 
//donde se encuentre
static unsigned char desencolar(struct _FIFO_1byte_ *f,unsigned char id) {
unsigned char *i;    
	if(colaVacia(f)){return TRUE;} // Cola vacía
    for(i=f->head;i<=f->tail;i++){//saca el id
        	if(*i==id){*i=0;}}
    f->ncount=0;
    for(i=f->head;i<=f->tail;i++){//recuenta los datos
    	  if(*i>0)f->ncount++;}
return TRUE;
}//----------------------------------------------------------------
...
// Función auxiliar para manejar la lógica de cada tarea
/* task:esta ocupado o pedido?, turno:quien sigue,*/
...
enum{CASO1=1,CASO2,CASO3,CASO4};
unsigned char e;

		if(*task != ID_vTask_OCUPADO && *turno != task_id){e=CASO1;goto e21;}
		if(*task == ID_vTask_OCUPADO && *turno != task_id){e=CASO2;goto e21;}
		if(*task != ID_vTask_OCUPADO && *turno == task_id){e=CASO3;goto e21;}
		if(*task == ID_vTask_OCUPADO && *turno == task_id){e=CASO4;}
e21:	switch(e){
		case CASO1:*task=ID_vTask_OCUPADO;return FALSE;//no esta ocupado no es mi turno
		case CASO2:return FALSE;
		case CASO3:*task=*recurso=ID_vTask_OCUPADO;return TRUE;
		case CASO4:*recurso=ID_vTask_OCUPADO;return TRUE;
		default:return FALSE;}//fin switch
}//-----------------------------------------------------------------
