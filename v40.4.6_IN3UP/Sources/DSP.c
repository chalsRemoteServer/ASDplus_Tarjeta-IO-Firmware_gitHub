...
//     dds.Bnderas.bit.debug=FALSE;
     ...
     zoom.factor=get_FactorDDS_zoom;//funcion que regresa el factor del zoom
     zoom.MaxZoom=Maximo_Zoom;//apuntador ala funcion que da los maximos en cada funcion
}// fin init DSP digital Signal Processor----------------------------------


//DISPLAY DE UN PUNTO EN EL MENU DDS++++++++++++++++++++++++++++++++++ 
/* Memoria inst:2 bytes,7:doubles, Pu:1 Pc:1
 * Pc: obtenemos las coordenadas uchar para el DDS
 * Pu: obtenemos los maximos del zoom respectivo,
 * d:6 es una instancia de 7 doubles.  */
unsigned char xDisplay_Punto_DDS_VFD(struct Coorduc *Puc,struct Coordusi *Pu,unsigned char *inst,double *d){
auto unsigned char estadoDDS,ret=0; 
const unsigned char PID_DDS_Point=PID11;

    
   
if(menu.contexto.Actual!=PANTALLA_DDS)return TRUE;	
if(menu.b.b.MenuPendiente) return TRUE; 	
if((Signal.CentX==0)&&(Signal.CentY==0)) return TRUE;
 estadoDDS=*inst;
 switch(estadoDDS){//visualizar señales
	 case 1://verficar que acabemos en menos de 1mseg el ciclo de DSP
	 case 2:estadoDDS++;break;//bandera de muestreo completo
	 case 3:*(d+0)=(double)Signal.CentX;estadoDDS++;break;//convertir los valores centrados en double
	 case 4:*(d+1)=(double)Signal.CentY;estadoDDS++;break;//convertir los valores centrados en doubles
	 case 5:if(Convert_2_DDS(Puc,Pu,inst+1,d)) estadoDDS++; break;
	 case 6:if(recurso.solicitar(_VFD_,PID11)) estadoDDS++; break;
	 case 7:if(vfd.f1.append(Puc->x,Puc->y,_PUNTO_))estadoDDS++; break;
	 case 8:recurso.devolver(_VFD_,PID11);estadoDDS++;break;
	 case 9:estadoDDS=0;ret=TRUE;break;
	default:estadoDDS=1;break;}//----------------------------------
*inst=estadoDDS;
return ret;
}//fin display point in DDS menu-----------------------------


/* Es un Hilo que despliega las lineas de sensibilidad en el
 * menu de DDS 
 *  MEMORIA 41+8:byte   6:usi  2+4+6:float   */
unsigned char xDisplay_Sens_DDS_VFD(unsigned char *inst,unsigned short int *usi,float *f){
auto unsigned short int S,SS,*usi1;//sensibilidad
//auto float phase;
auto unsigned char estado13,ret=0,*inst2;
auto float A,AA;//angulo
auto float *f2;
//auto unsigned char *inst1;
const unsigned char PID_displ_sens=PID12;

if(menu.contexto.Actual!=PANTALLA_DDS)return TRUE;
if(menu.b.b.MenuPendiente)return TRUE;

estado13=*(inst+0);//0
   inst2=inst+1;//40 bytes
       A=*f;    //0
      AA=*(f+1);//1
      f2=f+2;  //2,3..17
	   S=*(usi+0);
...
if(S==Deteccion.Sensibilidad){
  if(A==Deteccion.Phase){
		 return TRUE;}}	   
 switch(estado13){
   case 1:SS=Deteccion.Sensibilidad;AA=Deteccion.Phase;estado13++;break; //pause
   case 2:if(!Deteccion.bits11.x.MarcarAltura)estado13++;break;//sin Altura
   case 3:estado13++;break;//if(recurso.solicitar(_VFD_,PID12))estado13++;else ret=TRUE;break;
   case 4:if(display_Sens_Lim_v2(AA,SS,inst2,usi1,f2))estado13++;break;
   case 5:estado13++;break;
   case 6://recurso.devolver(_VFD_,PID12);se metio la gestion al hijo hilo
          ret=TRUE;
          dds.b.b.rePaintLines=0;S=SS;A=AA;
          estado13=0;break;
   default:estado13=1;break;
  }//fin estado-----------------------------------------------
  ...
  *(f+1)=AA;
return ret;	
}//fin de display de sensibilidad del menu de DDS del display VFD----------
...
void vTask5_Digital_Signal_Processor(void){
signed short int datax=0,datay=0; //where we save the ADC's now data	
static unsigned char estadoDDS;
static signed short int RawX0,RawY0;
static struct Coordf Pf[4];

static double instd[4];
static struct Coordusi Pu;
//auto struct Coorduc Pc;
static unsigned char status;//banderas para ejecutar gaficar en DDS o deteccion Procesamiento
const unsigned char PID_DDS_Point=PID11;
static unsigned char inst[27+27+2]; 


 switch(estadoDDS){//Deteccion.bits11.x.full=1 Hay datos ADC nuevos
	 case 1:if(Deteccion.bits11.x.full){Deteccion.bits11.x.full=0;estadoDDS++;}break;
	 case 2:DC_Blocker_Filter_v1(Signal.RawX,&Signal.CentX,&inst[0]);estadoDDS++;break;
	 case 3:DC_Blocker_Filter_v1(Signal.RawY,&Signal.CentY,&inst[28]);estadoDDS++;break;
	 case 4:
	 
	 //case 6:if(Detection_Y_Analisis_v2(Signal.CentX,Signal.CentY)){estadoDDS=10;}
	//        else{estadoDDS++;}break;
	 //case 7:if(Control_de_Rechazo()) estadoDDS++;break;
	 //case 8:Comm_DDS_GOTIS_FIFO(Signal.CentX,Signal.CentY);estadoDDS++;break;
	 default:estadoDDS=1;break;}
...
//sysmon2.i=1;controlador_LED3_Detect(BUSY);//funcionan los ADC
...
//          Generador_de_Señal_Debug_Tool(&Signal.CentX,&Signal.CentY);	   
		...
		//Rot.x=(float)Signal.Rotx;Rot.y=(float)Signal.Roty;
		//Memoria_Pixeles_DDS(Signal.Rotx,Signal.Roty,Deteccion.signalDDSx,Deteccion.signalDDSy);
		//Comm_DDS_GOTIS_FIFO(Signal.Rotx,Signal.Roty);
...
//acepta paso de parametros mas de 4 o 5, Si regresa falso no debe evaluarse el ajuste al centro. 
...
signed short int ssi;
	     if(dato==0)
	    	 return TRUE;  //cero no se calcula
	     if(dato<0) //es negativo???  
	     	   ssi=dato*-1;
	     else ssi=dato;
	     if(ssi>max)  //esta dentro del rango del calculo?
...
BD1:    if(Deteccion.tipo==NORMAL){//NORMAL det is when threshold lines is in horizolt without height lines
  ...
  if(vfd.x.ncount+10>vfd.x.size){
     switch(EstadoBarra){//conversion de bits individuales de boxes a numero de box
       case 0x0001:R=vfd.f1.append(1,0,_BOX_);/*FIFO_displayBox_push(1);*/ break;  //0000 0000 0000 0001
       case 0x0003:R=vfd.f1.append(2,0,_BOX_); break;  //0000 0000 0000 0011
       case 0x0007:R=vfd.f1.append(3,0,_BOX_); break;  //0000 0000 0000 0111
       case 0x000F:R=vfd.f1.append(4,0,_BOX_); break;  //0000 0000 0000 1111
       case 0x001F:R=vfd.f1.append(5,0,_BOX_); break;  //0000 0000 0001 1111
       case 0x003F:R=vfd.f1.append(6,0,_BOX_); break;  //0000 0000 0011 1111
       case 0x007F:R=vfd.f1.append(7,0,_BOX_); break;  //0000 0000 0111 1111
       case 0x00FF:R=vfd.f1.append(8,0,_BOX_); break;  //0000 0000 1111 1111
       case 0x01FF:R=vfd.f1.append(9,0,_BOX_); break;  //0000 0001 1111 1111
       case 0x03FF:R=vfd.f1.append(10,0,_BOX_); break;  //0000 0011 1111 1111
       case 0x07FF:R=vfd.f1.append(11,0,_BOX_); break;  //0000 0111 1111 1111
       case 0x0FFF:R=vfd.f1.append(12,0,_BOX_); break;  //0000 1111 1111 1111
       case 0x1FFF:R=vfd.f1.append(13,0,_BOX_); break;  //0001 1111 1111 1111
       case 0x3FFF:R=vfd.f1.append(14,0,_BOX_); break;  //0011 1111 1111 1111
       case 0x7FFF:R=vfd.f1.append(15,0,_BOX_); break;  //0111 1111 1111 1111
       case 0xFFFF:if(EstadoBarraMSB==OFF)
    	   	   	   	    R=vfd.f1.append(16,0,_BOX_);  //1111 1111 1111 1111
                   else R=vfd.f1.append(17,0,_BOX_);
                   break;
       case 0x0000:R=vfd.f1.append(0,0,_BOX_); break;
       default:break;
     }}//FIN SWITCH-------------------------------------------
...
//double anguloVibracionRad;
             ...
             if(!Deteccion.bits11.x.isAngVibracionInRad){
            	 Deteccion.bits11.x.isAngVibracionInRad=TRUE;
                 Deteccion.AnguloVibracionRad=(anguloVibracion*_2PI)/360; 
                 Deteccion.CosdAngulovibrRad=(signed short int)(cosd(Deteccion.AnguloVibracionRad));
                 Deteccion.SindAngulovibrRad=(signed short int)(sind(Deteccion.AnguloVibracionRad));}
	         //*xout=(signed short int)(xin*cosd(Deteccion.AnguloVibracionRad)+yin*sind(Deteccion.AnguloVibracionRad));
	         //*yout=(signed short int)((-1)*(xin)*sind(Deteccion.AnguloVibracionRad)+yin*cosd(Deteccion.AnguloVibracionRad));
             *xout=xin*Deteccion.CosdAngulovibrRad+yin*Deteccion.SindAngulovibrRad;
        	 *yout=(-1)*(xin)*Deteccion.SindAngulovibrRad+yin*Deteccion.CosdAngulovibrRad;
             __asm(nop);
#else      		
 double m,m1;
 double r;
...
//	  cambio_de_contexto(PANTALLA_DDS);
//	  dds.Bnderas.bit.DDS_Reload=FALSE;//repintar el DDS con los puntos guardados en los buffers
//	  dds.Bnderas.bit.clean_Buffers=FALSE;//NO BORRAR BUFERS
//	  //DDSpant->igxc2= ; //cursor donde va estar
//  }//FIN STATUS------------------------
//}//fin refresh_DDS_----------------------------------------------------------------
...
static signed short int Sx,Sy,i;
static struct Coordf Av;
double X,Y;
const unsigned short  int SAMPLE_NUM=13;
struct Coordusi M;
    X=(double)absf(P.x);
    Y=(double)absf(P.y);
    if(!init){init=1;i=1;Sx=0;Sy=0;
              Av.x=0;Av.y=0;
              return 0;}
    if(i==0){i=1;return 0;}
    if((X==0)||(Y==0)){return;}
    Sx+=X;Sy+=Y;
    Av.x=Sx/i;Av.y=Sy/i;
    if(i++>SAMPLE_NUM){init=0;
         M=Maximo_Zoom(z);//coordenadas maximas de ese zoom
         if((Av.x<(M.x/4))||(Av.y<(M.y/4))){
        	 zoom.Zoom=get_Fit_Zoom(Av);
        	 return 1;}}//HAy cambio de zoom
return 0;         
}//fin Monitor_ZoomIN---------------------------------------------------------------


/* este metodo se le ingresa las coordenadas y regresa el 
 * zoom mas acorde cercano al sus maximos del zoom*/
unsigned char get_Fit_Zoom(struct Coordf P){P.x=0;
//struct Coordssi P2;
          //P2.x=(signed short int)x;
          //P2.y=(signed short int)y;
//          zoom.Zoom=buscar_Nuevo_Zoom(P);
}//fin get_Fit_Zoom--------------------------------------------------------




///*buscar un zoom mayor al valor de las coordenadas que se ingresan
// *  regresa un numero de zoom */
//unsigned char buscar_Nuevo_Zoom(struct Coordf P){
//unsigned char i;
//struct Coordusi Max;
//       for(i=99;i>0;i--){
//    	   Max=Maximo_Zoom(i);  // MAXIMOs y minimos DEL CENTRADO
//    	   if(!isLimit_Overdue(P,Max)){
//    		   if(i>1)
//    			   return (i-1);
//    		   else return 1;}}
//}//fin buscar_Nuevo_Zoom-------------------------------------------------



/*controla y cambia automaticamente el zoom cuando
 *  esta en modo DDS  
 *  P coordenadas x&Y de los valores en tiempo real de deteccion
 *  zoom1, el zoom actual.
 *  Cambia el zoom Automaticamente si hay una señal que sobrepasa el zoom actual
 *  y si despues de 7seg hay señal muy chica que no sobrellena el zoom actual se
 *  reducel zoom automaticamente
 *  P ya debe estar centrada en 96,-64 */
//unsigned char Cambio_de_ZoomOUT(struct Coordf P,unsigned char zoom1){//------
//struct Coordusi P1;
//     P1=Maximo_Zoom(zoom1);  // MAXIMOs y minimos DEL CENTRADO
//	 if(isLimit_Overdue(P,P1)){//Se paso del limite impuesto por el zoom actual
////	     zoom.Zoom=buscar_Nuevo_Zoom(P);//busca nuevo zoom en base de un mayor valor
//	     return TRUE;}
//return FALSE;     
//}//fin de control de inteligencia de zoom-------------------------------------------  
//
///* es para ver si la coordenada P, se sobre pasa de los limites
// * Max., return True  si se sobre pasa de los limites
// * FALSE si no se sobrepasa de los limites*/
//unsigned char isLimit_Overdue(struct Coordf P,struct Coordusi lim){
//float x,y;
//     x=absf(P.x);y=absf(P.y);
//     if(x>lim.x)
//    	 return 1;
//     else{if(y>lim.y) return 1;
//     	  else{return 0;}}
//}//fin de es limite de sobre pasar los limites----------------------------------




/* Se multiplica por un factor depende del zoom1
 * regresa la coordenadad multiplicada por el factor  */
struct Coordf escalar_to_factor(struct Coordf P,unsigned char zoom1){
double zf,x,y;
struct Coordf P2;
           x=(double)P.x;
           y=(double)P.y;          
           zf=zoom.factor(zoom1);//Maximo_Zoom, get_FactorDDS_zoom
           zoom.zFactor=zf;
       	   P2.x=(float)(x/zf);
       	   P2.y=(float)(y/zf);       	   
return P2;
...
///* Analisa si las coordenada cabe en el zoom actual
// * seleccionado  */
//struct Coordf is_fit_in_zoom(struct Coordf P,
//	   unsigned char *inst,struct Coordusi *P2,
//	   unsigned char *ret2){
//auto double zf,x,y;
//auto struct Coordf P3;
//auto struct Coordusi *Max;
//auto unsigned char estado3;
//const unsigned char SI_CABE_EN_ZOOM=0x99;
//const unsigned char NEL_NO_EN_ZOOM =0x1A;
//
////auto struct Coordusi *P6;
//
//   Max=P2;
//
//  switch(estado3){
	   	            	  ...
	   	            	  if(xy>0){
	   	            		  if(xy>zoom.Maxy)
	   	            		     ...
	   	            		     return ((unsigned char)((-1*xy)+ZOOM_Y_CENTER));}//ec.recta (0,64),(64,0)
	   	            		         ...
	   	            		         ret= zoom.Cy;
	   	            		    ret= ((unsigned char)((-1*xy)+ZOOM_Y_CENTER));}	 //ec.recta(0,64),(-64,127)
	   	                  break;
...
/* velores maximos de DDS en funcion del zoom */
float  get_FactorDDS_zoom(unsigned char zoom1){	
struct Coordusi Max;
double Cx=96,Cy=64,rx,ry;
double x,y;
           Max=Maximo_Zoom(zoom1);
           x=(double)Max.x;
           y=(double)Max.y;
           rx=x/Cx;
           ry=y/Cy;
           if(absd(rx-ry)>1){
               ...
               __asm(nop);}
return ((float)rx);	
}//fin obtener maximos valores de DDS en funcino con el zoom
