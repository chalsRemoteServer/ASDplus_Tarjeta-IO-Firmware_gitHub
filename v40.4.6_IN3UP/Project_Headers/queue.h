...
struct DDS_GEN{
   unsigned char Status;// estado del init y monitor de señal	
   union _Byte91_{
...
struct _FIFO_1byte_v2{
	...
	unsigned char (*dequeue)(unsigned char *n,struct _FIFO_1byte_v2 *s);
...
struct _DISPLAY_VFD_{	
	...
	//struct _FIFO_1byte_  TX_IO;//fifo de recursos de tx serial
...
// Macros para crear y usar FIFOs fácilmente
// Ejemplo de uso:
//   FIFO_DECLARE(miFifo, 128);  // Declara una FIFO de 128 bytes
//   fifo.miFifo->Append(dato);   // Agregar dato
//   fifo.miFifo->Pop(&dato);      // Sacar dato

// Declaración de todas las FIFOs del sistema
typedef struct {
...
static unsigned char *shift_left(unsigned char *arr,unsigned char ncount);
