...
#elif(HW_VER_2==4)
  #pragma messasge("version de Hardware 2.3 pines que cambian")
...
#define VERSION2 "v40.4.7"



/*
los ADC se perdio la sincronia de los pulsos deSCLK cada 1mseg
 *             final=TT   VFD(LC) VFDmenu(LC) Memoria(LC) ADC(LC) DSP(LC)  Productdeanager(LC)  lineas de codigo=LC[.c,.h] tamaño total =TT(bytes)
 ...
 *              72000     186,47  4745,308    415,661:     206,31   18,15      20,20
 *  v36.1.0-73   10-NOV-16: sem43: Desarrollo d3spliegue de la señ	al en DDS, 
 ...
 *                  SE CAMBIO EL PROCESADOR, CAMBIO DE CHIP ERROR FALLA TRGET NO ERASE                                                
 ...
 v39.16.40.0-48        12-Mar-21 '9 depurando el bufer segundo en pantalla
...
* "v39.16.50.40-10"   24-agt-22, se perdieron 250 actualizaciones, ahora vamos a cambiar 
*                               la forma que se despliega numeros, posiciones y cuadritos al mismo tiempo
*                               y agregamos el DTR del display ala procesadora.     
*  v39.16.51.0-141    31-Agt-22, se controla el teclado por automatatas y se crea el seno cos ytangente
*                                por series de potencias de taylor maclaurin, actualizacion de control de errores                                
*  "v39.16.52.0-6"      27-sept-22 recuperamos mucha informacion perdida de las potencias mclauring y del control de errores
*                                 pero todavia no estan listos, se hizo el driver control principal de video ó graficacion
*                                 que se encarga de desplegar todo al vfd de manera operatica, ahora cambiamos de version
*                                 porque necesitamos cambiar el contrl de banderas de cambio de menu,y control DDS y control de Fifos
*                                 mas eficiente, lo pondremos en la clase menu.
*  "v39.16.53.36"   7-oct-22   vamos a cambiar el cambio de contexto de una mas controlada y operativa.
*                              y vamos a usar maquinas de estado mas controlada.                                                                                 
*  "v39.17.1.157"   18-Ene-23  DDS mover el angulo de phase en DDS.
*  "v39.17.2.172"   3-MAyo-23  Interconectando con el mismo SPI el DDS y los ADCs                
*  "v39.17.3.62"    8-Mayo-23  ya funcionan el generadr DDS y el monitor de errores de error reloj analogico no probado                
*  "v39.17.4.48"    12-May-23 cambio de procesador, ya no quiso comunicar al programador
*  "v39.17.4.48"    14-MAy-23 se redujo el proceso main a 16mseg desde 30mseg vamos a quitar la rotacion para cada coordenada de deteccion
*                              y solo la vamos a poner cuando esta el DDS vamos a poner la deteccion en la interrupcion
*                              y que dure menos de 500useg 
*  "v39.17.5.83"    31-May-23  logramos encender el sistema con Sistema operativo con maquinas de estados
*                               en portal inicio sistema operativo   S.O.  MaE  
*   "v39.19.5.24"  15-jun.23. tenemos en portal inicio la sensibilidad, phase rechazo y producto
*                               pero vamos a pasar el rechazo y el producto a lont int porque a los 65535 se detienen.
*   "v39.19.6.76"  23-jun-23  El portal inicio se puso en todos los numeros con sistema opertaivo menos la barra de deteccion.
*  "v39.19.7.51"    3-jul-23   se manda datos con el camando remoto del gotis se deienten con el comando de stop
*                              los datos se mandan y se parten y se vuelven a armar en las cordenadas que se enviaron.
*  "v39.19.8.18"    11-jul-23  estamos pasando el menu de DDS a modod con sistema operativo
*  "v39.19.11.1"               Cambios de Sistema Operativo
*  "v39.21.01.1"    30-jul.23  Pruebas con sistema OPerativo propio
*  "v39.22.01.1"    2-agt-23   Vamos a poner El DDS como portada para trabaja con detecion yen segundo plano trabajamo con
*                              el sistema operativo.
*  "v39.22.1.14"    13-agt-23  debugeando el sistema operativo otravez
*  "v39.22.3.39-1745" 28-agt-23   fallo el gestor de recursos nos regresamos a la version 39 y de alli saltamos al a version 5
*                                 la version 4 no funciono nos regresamos ala 3.39
*   v39.22.5.0  
*   "v39.22.7.45"    19-sept-23 portal inicio con sus parametros en movimiento luego pasa al DDS con todos sus parametros listos
*                               para se movidos por SO  por teclado.                                                                             
*   "v39.22.8.40"    28-sept-23 sistema operativo para cursos del menu dds, CURSORES Y MANEJO DE NUMEROS del menu DDS con SO. no depurado
*    "v39.22.9.53"    6-oct-23  menu DDS, cursores, numeros, y cambio de sensibilidad y sus lineas, phase y sens con numero up,down right,left
*   "v39.22.10.104"   21-nov-23   ya dibuja la deteccion en Desktop, la señal no se centra bien todavia en el DSP, todavia no
*                                  desplegamos puntos en DDS porque no se centra bien.
*     "v39.22.11.12"   se detuvo todo y se cambio el proyecto casi completamente
*  "v40.4.3"  se le quito todo lo grafico, tenemos deteccion y comando de alive y comando de 
*   		   X&Y, 
*  "v40.4.5"   comunicacion comando dalive y comandod de transmision x&y                        
*                        
*                                  
...
#define CONFIGURAR_ENTRADAS_DE_SISTEMA        0xD1
#define ID_COMUNICACIONES                     0xD2 
#define CONTROL_DE_FRECUENCIA                 0xD3
#define INFORMACION_DE_USUARIO                0xD4   
#define CONFIGURAR_ENTRADAS_DE_SISTEMA        0xD1
#define ID_COMUNICACIONES                     0xD2 
#define CONTROL_DE_FRECUENCIA                 0xD3
#define INFORMACION_DE_USUARIO                0xD4
#define CONTROL_PASSWORD                      0xD5
#define MENU_ADMINISTRATIVO                   0xD6
#define TIPO_DE_MAQUINA                       0xD7
#define GANANCIA_                             0xD8
#define FILTRO_                               0xD9
#define INGENIERIA_GRACIDA2                   0xDA
#define AJUSTE_PARAMETRICO_DE_PRODUCTO        0xDB
#define FRECUENCIA_SELECT                     0xDC
#define PARAMETROS_DE_DETECCION               0xDD
#define PANTALLA_DDS                          0xDE    
#define ERROR_LISTA                           0xDF
#define NOMBRE_PRODUCTO                       0xE0
#define RECORD_PRUEBAS                        0xE1
#define CONTROL_USER                          0xE2
#define CONTROL_DE_FRECUENCIA2                0xE3
#define INIZIALIZAR_MEMO                      0xE4
#define PRUEBAS_RUIDO                         0xE5
...
#define FX   94 //ascii del cursor de la pantalla de Texto processor
#define S1   0xA0
#define S2   0xA1
#define S3   0xA2
#define S4   0xA3
...
//MENU PORTAL_INICIO MENU KONSTANTES++++++++++++++++++++++++++++++++
#define POS_X_SENS       80
#define POS_X_PHASE      176
#define POS_X_CNTA_RECH  152
#define POS_X_CNTA_PROD  152
//FIN DE MENU PORTAL_INICIO MENU KONSTANTES++++++++++++++++++++++++++++++++
...
#define SIZE_LINE 25//maximos numero de char por linea en DISPLAY VFD
#define SIZE_PID   0x08//Tamaño de control DE PID
...
struct ArbolMenu{
    ...
    unsigned char (*func1)(void);//nuevo a puntador a funcion que despliegua el menu actual
