...
// Para 1 ms a 58.8 MHz
// 58,800 ciclos = 1 ms
void delay_ms(unsigned int ms) {
    unsigned int i;
    for(i = 0; i < (ms * 58800); i++) {
        __asm(nop);
    }
}//--------------------------------------------------
...
/*regresa los Tick del sistema se incrementa cada 9.7useg */
unsigned long int xTaskGetTickCount(void){
	return xTickCount;
}//fin de get task get tick count

// Función para obtener tiempo actual en microsegundos
