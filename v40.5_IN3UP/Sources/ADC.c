...
#define  INIT_FLAG 0xA0 //bandera para indicar que se inizio el ADC
...
static volatile unsigned char busy_interrupt;
...
register unsigned char i;	
     ...
     setReg16(QMR, 0x810AU);//1000 0001 0000 1010)  ADC:(1000 0000 0001 0000)  ->16bits tansfer
	 Busy_Disable();//Disable interrupt Busy
	 setReg16(QWR,0x0100); //0000 0001 0000 0000
	 setReg16(QAR,0x0020);//set pointer to save data to transmit
	 setReg16(QDR,0xE000);//1110 0000 0000 0000 data to transmit
	 setReg16(QDR,0xE000);
	 setReg16(QDLYR,0x0707);
	 Select_SPI_device(ADC_CH0);
	 ...
	 TI1_EnableEvent();
	 TI1_Enable();
	 Busy_Enable();//habilita la interrupcion  busy VA AN READ CONVERT SINO  BORRALA BANDERA PEN..
	 ADCstatus=0xC0 | ADC_1;//abxx xxxx a=anable ADC b=flag of RX SPI
	 RC_ON(); //no quitar de aqui
}//inicizamos el ADC---------------------------------------------------------------------------------------

//ADC controller
void vTask6_ADC_controller(void){
static unsigned char estado6,estado6B,init_ADC;
  switch(estado6){
	  case 1:if(init_ADC_Regs(&init_ADC))estado6++;break;
	  case 2:if(SystemBoot){Busy_Enable();estado6++;}break;
	  case 2:if(busy_interrupt
	case x:estado6B=0;
		   init_ADC=INIT_FLAG;break;
	default:estado6B=1;break;}	
}  
N1:switch(estado6){  
...
}//fin vTask6 ADC controller---------------------


unsigned char init_ADC_Regs(unsigned char *init){
	setReg16(PQSPAR,0x1515);//0001 0101 0001 0101=1515H
	setReg16(QMR, 0x810AU);//1000 0001 0000 1010)  ADC:(1000 0000 0001 0000)  ->16bits tansfer
	Busy_Disable();//Disable interrupt Busy
	setReg16(QWR,0x0100); //0000 0001 0000 0000
	...
	px8=initx8;
	px7=initx7;
	TI1_EnableEvent();//habilitamos timer1
	TI1_Enable();
	ADCstatus=0xC0 | ADC_1;//abxx xxxx a=anable ADC b=flag of RX SPI
	RC_ON();
	*init=INIT_FLAG;
return	TRUE;
}//fin de iniziar los ADC regs-----------------------
...
volatile unsigned char i;
	   ...
	   for(i=0;i<14;i++) //7useg
               __asm(nop);
       Select_SPI_device(ADC__CH0); //Ch1 tiene una duracion de 7useg
       __asm(nop);__asm(nop);__asm(nop);__asm(nop);__asm(nop);__asm(nop);
       setReg16(QAR,0x0010);//apuntar a direccion 10h
       __asm(nop);__asm(nop);__asm(nop);__asm(nop);__asm(nop);__asm(nop);
       Signal.RawX=(signed short int)getReg16(QDR);//leer direccion 10h
       __asm(nop);__asm(nop);__asm(nop);__asm(nop);__asm(nop);__asm(nop);
       Signal.RawY=(signed short int)getReg16(QDR);//leer direccion 11h
       if(Deteccion.bits11.x.full){
    	        __asm(nop);
    	        __asm(nop);}
       else Deteccion.bits11.x.full=TRUE;
}//fin busy interrupt  INTERRUPT REQUEST-------Busy_OnInterrupt--------------------------------------------------------------
 

/* * */
unsigned char Read_QSPI2(signed short int *xx8,signed short  *xx7){
static unsigned char i;
//unsigned char n=0;

    if(Deteccion.bits11.x.full){Deteccion.bits11.x.full=0;
         *xx8=Deteccion.RawX;
         *xx7=Deteccion.RawY;
         return TRUE;}
    else return FALSE;
...
const unsigned char len=4,SIZE_TRAMA=8;
...
l21:switch(estado){
    	...
    	case 7:datos[3]=(unsigned char)(x >> 8);// Enviar BYTE_XH (byte alto de X)
               ...
               datos[7]=ETX;estado++;i=0;break;//Enviar BYTE_YL
    	case 8:if(vfd.TxDisp.appendByte(datos[i],&vfd.TxDisp))estado++;break;
	...
	if((ADCstatus & 0xC0)==0xC0){//1xxx xxxx bit que habilita la ejecucion de los ADC's
			 //if(swap==0xAA){//como tm1 se ejecuta cada 520nseg esto hace que se ejecute cada 1mseg
				// swap=0;// quitamos la bnadera para hacer flip flop y generar 1ms de interrupcion
				 ...
				 __asm(nop);
				 __asm(nop);
				 RC_ON();//RC_OFF();//640nseg, que se ejecute la conversion, se ordena la execucion con low
	             }//PULSO DE 230ns, nanosegundos
			 //else {swap=0xAA;}
